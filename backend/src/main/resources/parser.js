...
var module = (function (parser) {
    ...
    * Return AST
    */
    function checkAndAddLimit(ast, limit) {

        if (!ast) return ast

        if (ast && ast.type == "Select" && ast.limit == null) {
    ...
    return {
        ...
        checkAndAddLimitClause: function (ast, limit) {
            ast.value = checkAndAddLimit(ast.value, limit)
            var updatedQuery = parser.stringify(ast)
            return updatedQuery
        },
